...
func main() {
	...
	//   - Lighter 交易对精度处理与 Nonce 管理 (SkipNonce)
	//   - Lighter 链下签名机制 (Schnorr/Goldilocks)
	ex, err := exchange.NewLighterAdapter(&cfg.Exchange, bus)
	if err != nil {
		utils.Logger.Fatal("Lighter 适配器创建失败", zap.Error(err))
	}

	// 启动适配器（WebSocket 连接 + 心跳 + 事件桥接）
	if err := ex.Start(context.Background()); err != nil {
		utils.Logger.Fatal("Lighter 适配器启动失败", zap.Error(err))
	}
