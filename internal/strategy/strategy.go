...
type MartingaleStrategy struct {
	...
	// ★ 周期代际：每次新周期入场（IDLE→PLACING_GRID）时递增。
...
func NewMartingaleStrategy(cfg *config.StrategyConfig, ex exchange.ExchangeAdapter, bus *core.EventBus) *MartingaleStrategy {
...
func (s *MartingaleStrategy) handleOrderUpdate(ctx context.Context, event core.Event) error {
	...
	if order.Status == "FILLED" {
		...
		if order.Side == exchange.OrderSideBuy {
			...
			// ★ 修复：不再立即重置 FSM。先轮询确认持仓真正归零：
			//   - 防止止盈部分成交（残留仓位）时误重置，导致新周期叠在残留仓上
			//   - 防止持仓 API 短暂滞后时误以为周期结束
			//   - orderUpdates 事件即使丢失/延迟，monitorPositionStatus 5s 轮询兜底
			s.mu.RLock()
			cid := s.cycleID
			s.mu.RUnlock()
...
// resetToIdle 将 FSM 状态重置为 IDLE（带周期代际校验）。
// 返回是否实际执行了重置；只有实际重置时才应发起撤单。
// cleanCycleAndResetToIdle 异步清理当前周期的遗留订单，然后重置状态为 IDLE。
...
func (s *MartingaleStrategy) cleanCycleAndResetToIdle(cycleID uint64) {
	...
	state := s.currentState
	s.mu.RUnlock()
	if current != cycleID || state == StateIdle {
		return
	}
...
// waitPositionZeroThenReset 卖单成交后轮询持仓，确认归零才重置 FSM。
//
// 背景与动机：
//   - 原实现收到 SELL FILLED 立即重置 IDLE 并撤单；若止盈部分成交（残留仓位）
//     或持仓 API 短暂滞后，提前重置会让新周期叠在残留仓上。
//   - orderUpdates 的 SELL 事件即使丢失/乱序，monitorPositionStatus 5s 轮询
//     和 handlePositionUpdate 仍是兜底重置路径。
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if err != nil {
			...
			utils.Logger.Debug("等待持仓归零：仍有持仓（可能部分成交）",
				zap.Float64("size", pos.Size), zap.Int("attempt", attempt+1))
		}
...
func (s *MartingaleStrategy) enterLong(currentPrice float64) error {
	...
	allocatedChunk := s.calcAllocatedChunk()
	unitQtyRaw := allocatedChunk / currentPrice
	...
	baseQty := unitQty * 0.06 // ★ 恢复用户自定义：首仓占用 6%
	...
	// ★ 修复：与交易所真实最小要求（MinOrderValue）对比
	if baseQty*currentPrice < MinOrderValue {
		qtyNeeded := MinOrderValue / currentPrice
		...
		utils.Logger.Info("基础下单金额不足，提升至交易所最低限制",
			zap.Float64("adjusted_qty", baseQty),
			zap.Float64("value", baseQty*currentPrice),
			zap.Float64("min_order_value", MinOrderValue))
	}
...
func (s *MartingaleStrategy) placeGridOrders() {
	...
	unitQty := utils.FloorToTickSize(allocatedChunk/entryPrice, s.stepSize)
...
func (s *MartingaleStrategy) updateTP() {
	...
	// ★ 修复：防止重建/修改 TP 时价格已经可成交（重启后瞬间平仓）。
	// 场景：上次退出时 main 的优雅关闭撤掉了全部挂单（含 TP），重启后发现
	// 有持仓但无 TP；若此时市价已 ≥ 持仓均价×1.008，重建的 SELL LIMIT 会
	// 立即成交，用户持仓在重启瞬间被平掉。这里校验市价，价格可成交时
	// 跳过本次更新，等价格回落后由后续 updateTP 恢复止盈保护。
	marketPrice, priceErr := s.exchange.GetLatestPrice()
	if priceErr != nil {
		utils.Logger.Warn("获取市价失败，跳过本次 TP 更新（防止追价成交），等待下次重试",
			zap.Error(priceErr))
		return
	}
...
// calcAllocatedChunk 动态计算最低下单金额
func (s *MartingaleStrategy) calcAllocatedChunk() float64 {
	balance, err := s.exchange.GetBalance()
	if err != nil {
		utils.Logger.Error("获取余额失败，使用 MinOrderValue", zap.Error(err))
		return MinOrderValue
	}
	// ★ 修复：用户希望直接使用 0.06 代表 6% 的总资金，
	// 因此基准金额 (Allocated Chunk) 必须是 100% 的当前总余额。
	// 忽略 config 中的 BaseRatio，防止 0.05 * 0.06 = 0.3% 的微缩错误。
	notional := balance
	...
	utils.Logger.Info("基准仓位额度计算 (Allocated Chunk)",
...
//	...  1.16   后续层使用最后一层倍数
//
// 首仓使用 enterLong 中硬编码的 0.06 (6%)，不通过此函数。
